...
import { Calendar, CheckCircle2, Clock, ExternalLink, Search, FileText, FileDown, LayoutGrid, List } from 'lucide-react';
...
export default function TaskListTab({ tasks, onUpdateClick }: { tasks: any[], onUpdateClick: (id: string) => void }) {
  ...
  const getStatusLabel = (task: any) => {
    // Si ya está marcada como vencida explícitamente en la BD:
    ...
    // Cálculo dinámico si la fecha pasó y no está cumplida/cancelada
  ...
  const getStatusColor = (task: any) => {
    ...
    return 'bg-blue-50 text-blue-800 border-blue-200 dark:bg-blue-900/30 dark:text-blue-400 dark:border-blue-900'; // Pendiente
  };
  ...
  const exportPDF = () => {
    ...
    const tableData = filteredTasks.map(t => [
      t.id, t.nombreAsignado + ' ' + t.apellidoAsignado, t.area, t.tipo, getStatusLabel(t), t.fechaVencimiento
    ]);
    
    autoTable(doc, {
      startY: 20,
      head: [['ID', 'Asignado a', 'Área', 'Tipo', 'Estado', 'Vencimiento']],
      body: tableData,
      theme: 'grid',
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input
            type="text"
            placeholder="Buscar por descripción, tipo, estado o ID..."
            value={searchTerm}
